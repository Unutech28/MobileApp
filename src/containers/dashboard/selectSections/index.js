...
class SelectSection extends Component {
  ...
  componentDidMount() {
    console.log("Stage Props===>", this.props);
    ...
    this.setState({
      ...
      selectedStage: this.props.selectedCurruntStage,
  ...
  _onProceedClick1 = (stageName = "", stagedata) => {
    ...
    let data = {
      ...
      stage: this.props.selectedProgram.stage,
      stageDay: this.state.selectedStage ? this.state.selectedStage : 1,
    };

    console.log("data==>", data);
    this.props.AppActions.setCardLimit(20);
    this.props.AppActions.getCardsList(
      data,
      0,
      (res) => {
        if (res.length == 0) {
  ...
  render() {
    ...
    console.log("new stage props==>", this.props);
    ...
    return (
      <View style={{ flex: 1, zIndex: 1, backgroundColor: COLOR.WHITE }}>
        ...
        {this.state.stage && this.state.stageName && (
          ...
          >
            {this.props.stage.sectionName.map((element, index) => {
              return (
                <TouchableOpacity
                  onPress={() => {
